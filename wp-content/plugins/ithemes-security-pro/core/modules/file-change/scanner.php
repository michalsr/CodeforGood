...
final class ITSEC_File_Change_Scanner {
	...
	public function get_email_report( $email_details ) {
		...
		$report  = '<strong>' . __( 'Scan Time:', 'it-l10n-ithemes-security-pro' ) . '</strong> ' . date( 'l, F jS g:i a e', $itsec_globals['current_time'] ) . "<br />" . PHP_EOL;
	...
	private function send_notification_email( $email_details ) {
		...
		if ( ! ITSEC_Modules::get_setting( 'global', 'digest_email' ) ) {
			...
			$subject = '[' . get_option( 'siteurl' ) . '] ' . __( 'WordPress File Change Warning', 'it-l10n-ithemes-security-pro' ) . ' ' . date( 'l, F jS, Y \a\\t g:i a e', $itsec_globals['current_time'] );
