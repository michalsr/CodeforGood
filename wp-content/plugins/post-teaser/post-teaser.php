...
class post_teaser {
	...
	function post_teaser() {
		...
		$this->default_options['teaser_template'] = __('<a href="%permalink%" title="Permanent Link: %plain_title%" rel="bookmark">%title% full post</a><p>(%word_image_count%, estimated %reading_time% reading time)</p>', 'post-teaser');
		...
		$this->default_options['home_showall'] = '1';
	...
	function init_option_page() {
		...
		$post_teaser_page = add_options_page(__('Post Teaser', 'post-teaser'), __('Post Teaser', 'post-teaser'), 'manage_options', __FILE__, array(&$this, 'option_page'));
...
//]]>
	...
	function option_page() {
			...
			<div style="background-color: rgb(207, 235, 247);" id="message" class="updated fade"><p><?php _e('Settings saved.', 'post-teaser') ?> <a href="<?php echo get_option('siteurl'); ?>"><?php _e('View site &#187;', 'post-teaser') ?></a></p></div>
			<?php endif; ?>
			...
			<form action="<?php echo htmlspecialchars($_SERVER["REQUEST_URI"]); ?>" method="post" name="post_teaser_form" id="post_teaser_form">
				...
				<?php if ($this->more_link == '1'): ?>
					<input type="hidden" name="more_link" id="hidden_more_link" value="" />
				...
				<?php if ($this->block_stats == '1'): ?>
					<input type="hidden" name="block_stats" id="hidden_block_stats" value="" />
				...
				<?php if ($this->block_stats_before == '1'): ?>
					<input type="hidden" name="block_stats_before" id="hidden_block_stats_before" value="" />
				...
				<?php if ($this->omit_fullpost == '1'): ?>
					<input type="hidden" name="omit_fullpost" id="hidden_omit_fullpost" value="" />
				...
				<p><em>
				<?php _e("The filters below follows the precedence (priority) as it is ordered below if there are confilcts in the filters setting", 'post-teaser') ?>
				<?php echo "<br />" ?>
				<?php _e("(i.e. the first filter overwrites the second and so on)", 'post-teaser') ?>
				</em></p>

				<p><label>
				<?php if ($this->home_control == '1'): ?>
					<input type="hidden" name="home_control" id="hidden_home_control" value="" />
				...
				<?php if ($this->use_disable_filter == '1'): ?>
					<input type="hidden" name="use_disable_filter" id="hidden_use_disable_filter" value="" />
				...
				<?php if ($this->disable_inverse == '1'): ?>
					<input type="hidden" name="disable_inverse" id="hidden_disable_inverse" value="" />
				...
				<?php if ($this->disable_cat_always == '1'): ?>
					<input type="hidden" name="disable_cat_always" id="hidden_disable_cat_always" value="" />
				...
				<?php if ($this->disable_tag_always == '1'): ?>
					<input type="hidden" name="disable_tag_always" id="hidden_disable_tag_always" value="" />
				...
				<?php if ($this->use_showall_filter == '1'): ?>
					<input type="hidden" name="use_showall_filter" id="hidden_use_showall_filter" value="" />
				...
				<?php if ($this->showall_inverse == '1'): ?>
					<input type="hidden" name="showall_inverse" id="hidden_showall_inverse" value="" />
				...
				<?php if ($this->showall_cat_always == '1'): ?>
					<input type="hidden" name="showall_cat_always" id="hidden_showall_cat_always" value="" />
				...
				<?php if ($this->showall_tag_always == '1'): ?>
					<input type="hidden" name="showall_tag_always" id="hidden_showall_tag_always" value="" />
				...
				<?php if ($this->zero_counts == '1'): ?>
					<input type="hidden" name="zero_counts" id="hidden_zero_counts" value="" />
